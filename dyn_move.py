...
# Chris Glomb, Christian Gibbons, Michael Reyes
# In Class Assignment 3
# 11/18/14

# Modified from dyn_move_example.py by Daniel M. Lofaro
...
def main(settings):
    ...
    current_pitch = 0
    while True:
        [status, framesize] = d.get(dmxl, wait=True, last=True)
        
        for actuator in myActuators:
            if ( actuator.id == 2):
                actuator.moving_speed = dmxl.pitch_speed
                actuator.goal_position = rad2dyn(dmxl.pitch_dir)
            elif ( actuator.id == 3):
                actuator.moving_speed = dmxl.yaw_speed
                actuator.goal_position = rad2dyn(dmxl.yaw_dir)
        net.synchronize()
        ...
        for actuator in myActuators:
            ...
            print 'ID#',actuator.id,' ps = ', dmxl.pitch_speed, 'ys = ', dmxl.yaw_speed, '  enc = ', enc
